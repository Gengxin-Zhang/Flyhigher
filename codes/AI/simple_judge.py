...
class ClientA(WebSocketClient):
    def opened(self):
        self.send("{'op': 'auth', 'appid': 'ABC', 'name': 'XHH', 'color_a':100, 'color_r':120,  'color_g':80, 'color_b':210}")
...
if __name__ == '__main__':
    try:
        ws = ClientA('ws://localhost:9001/websocket', protocols=['chat'])
        ws.connect()
        ws.run()
